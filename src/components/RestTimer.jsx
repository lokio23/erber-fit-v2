...
export default function RestTimer({ seconds, onDone, onSkip }) {
    ...
    return (
      <div className="fixed bottom-[calc(4rem+env(safe-area-inset-bottom))] left-0 right-0 z-50 px-4 pb-2">
        <button
          ...
          className="w-full flex items-center justify-between bg-card border border-accent/30 rounded-full px-4 py-2.5 shadow-lg shadow-black/50"
        >
  ...
  return (
    <div className="fixed bottom-[calc(4rem+env(safe-area-inset-bottom))] left-0 right-0 z-50 px-4 pb-2">
      <div className="bg-card border border-border rounded-2xl px-5 py-4 shadow-xl shadow-black/60">
        {/* Progress bar */}
        ...
        <div className="text-center">
          <p className="font-display text-6xl text-accent tracking-wider leading-none">
            {formatTime(timeLeft)}
          </p>
        ...
        <div className="flex items-center justify-center gap-3 mt-4">
          <button
            onClick={() => extend(-30)}
            className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-bg border border-border text-xs font-mono text-muted hover:text-text transition-colors"
          >
            <Minus size={12} /> 30s
          </button>
          <button
            onClick={onSkip}
            className="px-5 py-1.5 rounded-lg bg-accent/10 border border-accent/20 text-xs font-mono text-accent hover:bg-accent/20 transition-colors"
          >
            Skip
          </button>
          <button
